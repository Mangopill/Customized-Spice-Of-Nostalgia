...
import mangopill.customized_spice_of_nostalgia.common.core.network.CSONData;
...
public class PlayerFoodData extends FoodData {
    ...
    protected void add(int foodLevel, float saturationLevel) {
        ...
        ModifiedFoodData modifiedFoodData = getResult(foodLevel, saturationLevel);
    ...
    public void sync() {
        if (player instanceof ServerPlayer serverPlayer) {
            PacketDistributor.sendToPlayer(serverPlayer, new CSONData(new PlayerFoodDataPack(recentAdd, foodExpectation, saturationExpectation)));
        }
    }
    ...
    public record PlayerFoodDataPack(Queue<Pair<Integer, Float>> recentAdd, float foodExpectation,
            ...
            public static final StreamCodec<ByteBuf, PlayerFoodDataPack> STREAM_CODEC = StreamCodec.composite(
                    createQueueStreamCodec(), PlayerFoodDataPack::recentAdd,
                    ByteBufCodecs.FLOAT, PlayerFoodDataPack::foodExpectation,
                    ByteBufCodecs.FLOAT, PlayerFoodDataPack::saturationExpectation,
                    PlayerFoodDataPack::new
            );

        private static StreamCodec<ByteBuf, Queue<Pair<Integer, Float>>> createQueueStreamCodec() {
                StreamCodec<ByteBuf, Pair<Integer, Float>> pairStreamCodec = StreamCodec.composite(
                        ByteBufCodecs.VAR_INT, Pair::getFirst,
                        ByteBufCodecs.FLOAT, Pair::getSecond,
                        Pair::of
                );
                return ByteBufCodecs.collection(size -> new LinkedList<>(), pairStreamCodec);
            }
